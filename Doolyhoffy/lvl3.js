  ...
  switch (event.keyCode) {
    ...
    case 65:
      if (!maze.cells[player.col][player.row].westWall) {
        ...
        if(player.col == maze.rows - 1 && player.row == maze.rows - 1)
        {
    ...
    case 87:
      if (!maze.cells[player.col][player.row].northWall) {
        ...
        //console.log(maze.cells.length -1);
        //console.log(maze.rows - 1);
      }
      break;
    default:
      break;
...
function onLoad() {
  ...
  player = new Player();
  maze = new Maze(32, 32, 25);

  document.addEventListener("keydown", onKeyDown);
}

function myFunction() {
  document.getElementById("demo").innerHTML = "<a href='https://jschooltink.gcmediavormgeving.nl'>CODE: yeyIEscaped</a>";
}
